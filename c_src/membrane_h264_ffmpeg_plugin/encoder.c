...
UNIFEX_TERM encode(UnifexEnv *env, UnifexPayload *payload, int64_t pts, int shared_payload, State *state) {
  ...
  res = get_frames(env, frame, &out_frames, &dts_list, &max_frames, &frame_cnt, shared_payload, state);
...
UNIFEX_TERM flush(UnifexEnv *env, int shared_payload, State *state) {
  ...
  int res = get_frames(env, NULL, &out_frames, &dts_list, &max_frames, &frame_cnt, shared_payload, state);
  switch (res) {
  case ENCODER_SEND_FRAME_ERROR:
    res_term = encode_result_error(env, "send_frame");
    break;
