defmodule Membrane.H264.FFmpeg.Parser.Test do
  ...
  describe "Check if H264.RemoteStream stream_format" do
    test "is parsed correctly" do
      state = init_pipeline()
      assert state.frame_prefix == @expected_prefix
    end

    test "allows the Parser to insert missing pps and sps" do
      state = init_pipeline()

      assert {actions, %{frame_prefix: <<>>}} =
    ...
    test "has lower priority than in-band parameters" do
      state = init_pipeline()
  ...
  test "Check if Parser won't crash if short first buffer is sent" do
    ...
    assert {[], new_state} = Parser.handle_process(:input, %Buffer{payload: payload1}, nil, state)

    assert {actions, %{frame_prefix: <<>>}} =
             Parser.handle_process(:input, %Buffer{payload: payload2}, nil, new_state)

    assert [stream_format: {:output, _format}, buffer: {:output, buffers}] = actions
  ...
  defp init_pipeline() do
    ...
    assert {[], state} = Parser.handle_setup(nil, state)

    assert {[], state} = Parser.handle_stream_format(:input, @input_stream_format, nil, state)
    state
  end
